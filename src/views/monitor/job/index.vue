    ...
    <el-dialog :title="title" v-model="open" width="820px" append-to-body>
      <el-form ref="jobRef" :model="form" :rules="rules" label-width="120px">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="采集周期" prop="cronExpression">
              <el-input
                :value="displayCron"
                :title="form.cronExpression || ''"
                disabled
              >
                <template #append>
...
// 显示给用户的 cron 文本（翻译后的），但不改变提交用的 form.cronExpression
const displayCron = computed(() => {
  const raw = form.value.cronExpression;
  if (!raw) return "";
  try {
    return translateCron(raw);
  } catch (e) {
    return raw;
  }
});
