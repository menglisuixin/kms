  ...
  <div>
    ...
    <div class="popup-main">
      <div class="popup-result">
        ...
        <!-- 简短说明：将当前选中各字段的含义以小字形式展示 -->
        <div class="cron-hint">
          当前表达式含义：{{ translateCron(crontabHumanReadable) }}
        </div>
      </div>
...
import translateCron from "@/utils/cron.js";
...
// 用于展示一个简短的人类可读说明（覆盖常见情况）
const crontabHumanReadable = computed(() => {
  const v = crontabValueObj.value;
  // 简单规则匹配：* 表示任意，? 表示不指定
  const s = v.second;
  const m = v.min;
  const h = v.hour;
  ...
  if (
    ...
    // 上面第一种覆盖每分钟在 0 秒触发，第二种已被每秒覆盖，所以这里判断更具体的每分钟场景
    if (s === "0") return "每分钟的 0 秒执行一次";
  }
  ...
  if (
    ...
    return `每天 ${h} 时 ${m} 分 执行`;
  }

  // 指定周执行
  if (s === "0" && m !== "*" && h !== "*" && w && w !== "?" && w !== "*") {
    return `每周 ${w} 的 ${h} 时 ${m} 分 执行`;
  }
  ...
  if (
    ...
    return `每月 ${d} 日 ${h} 时 ${m} 分 执行`;
  }

  // 否则返回表达式简短摘录
  let parts = [s, m, h, d, mon, w];
  if (y) parts.push(y);
  return parts.join(" ");
});
